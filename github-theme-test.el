...
(github-test-assert (boundp 'github-flavor)
                    "github-flavor is defined")
(github-test-assert (boundp 'github-flavor-alist)
                    "github-flavor-alist is defined")
(github-test-assert (listp github-flavor-alist)
                    "github-flavor-alist is a list")

(message "\n--- Test 2: All flavors are defined ---")
(github-test-assert (assoc 'light github-flavor-alist)
                    "light flavor exists")
(github-test-assert (assoc 'light-high-contrast github-flavor-alist)
                    "light-high-contrast flavor exists")
(github-test-assert (assoc 'dark github-flavor-alist)
                    "dark flavor exists")
(github-test-assert (assoc 'dark-dimmed github-flavor-alist)
                    "dark-dimmed flavor exists")

(message "\n--- Test 3: Color retrieval works ---")
(setq github-flavor 'dark)
(github-test-assert (string= "#e6edf3" (github-color 'fg-default))
                    "fg-default for dark is #e6edf3")

(setq github-flavor 'light)
(github-test-assert (string= "#1f2328" (github-color 'fg-default))
                    "fg-default for light is #1f2328")

(setq github-flavor 'dark-dimmed)
(github-test-assert (string= "#adbac7" (github-color 'fg-default))
                    "fg-default for dark-dimmed is #adbac7")

(setq github-flavor 'light-high-contrast)
(github-test-assert (string= "#0e1116" (github-color 'fg-default))
                    "fg-default for light-high-contrast is #0e1116")

(message "\n--- Test 4: Light/dark detection works ---")
(setq github-flavor 'light)
(github-test-assert (github--light-p)
...
(setq github-flavor 'dark)
(github-test-assert (not (github--light-p))
...
(github-test-assert (string= "#ffffff" (github-lighten "#000000" 100))
                    "Lighten black by 100% gives white")

(github-test-assert (string= "#000000" (github-darken "#ffffff" 100))
                    "Darken white by 100% gives black")

(github-test-assert (string= "#808080" (github-lighten "#000000" 50))
                    "Lighten black by 50% gives gray")

(message "\n--- Test 6: Color quantization works ---")
(let ((quantized (github-quantize-color "#1f2328")))
  (github-test-assert (string-match-p "^#[0-9a-f]\\{6\\}$" quantized)
                      "Quantized color is valid hex format"))

(message "\n--- Test 7: Custom color setting works ---")
(setq github-flavor 'dark)
(let ((original (github-color 'fg-default)))
  (github-set-color 'fg-default "#ff0000")
  (github-test-assert (string= "#ff0000" (github-color 'fg-default))
                      "Custom color can be set")
  ;; Restore original
  (github-set-color 'fg-default original))
...
(dolist (flavor '(light light-high-contrast dark dark-dimmed))
  ...
  (dolist (color '(fg-default fg-muted canvas-default canvas-subtle
    ...
    (github-test-assert (github-color color)
